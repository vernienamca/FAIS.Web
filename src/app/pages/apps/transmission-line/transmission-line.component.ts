...
export class TransmissionLineComponent implements OnInit, OnDestroy {
  form: FormGroup; 
  ...
  save(): void {
    ...
    this._portalService.updateTransmissionLine(data)
      .pipe(takeUntil(this._onDestroy$))
      .subscribe(data => {
        if (!data) {
          return;
        }
        let snackBarRef = this._snackBar.open('Transmission line profile has been successfully updated.', 'Close');
